...
pub struct NotFindPageTableBaseError {}

impl PageTableTansform {
    pub fn new(random_page_table: bool) -> Result<Self, NotFindPageTableBaseError> {
        ...
        if random_page_table {
            ...
            if !pml4t_va.is_null() {
                ...
                while unsafe {
                    ...
                    index += 1;
                    if index >= 512 {
                        return Err(NotFindPageTableBaseError {});
                    }
                }
    ...
    pub fn get_pte_address(self, addr: u64) -> u64 {
        (((addr & 0xffffffffffff) >> 12) << 3) + self.pte_base
    }

    pub fn get_pde_address(self, addr: u64) -> u64 {
        (((addr & 0xffffffffffff) >> 21) << 3) + self.pde_base
    }

    pub fn get_ppe_address(self, addr: u64) -> u64 {
        (((addr & 0xffffffffffff) >> 21) << 3) + self.ppe_base
    }

    pub fn get_pxe_address(self, addr: u64) -> u64 {
        (((addr & 0xffffffffffff) >> 21) << 3) + self.pxe_base
    }
}
