pub mod ept {
    ...
    impl EptState {
            ...
            *new_entry = set_bits_value(
            ...
            for i in 0..self.number_of_enabled_memory_ranges {
                if address_of_page <= self.memory_ranges[i as usize].physical_end_address {
                    if (address_of_page + 512 * PAGE_SIZE as u64 - 1)
        ...
        fn ept_logical_processor_initialize(&mut self) {
            ...
            let pfn =
                virtual_address_to_physical_address(((&mut pml3[0]) as *mut u64) as *mut c_void)
                    / PAGE_SIZE as u64;
            ...
            for i in 0..pml3.len() {
                let pfn = virtual_address_to_physical_address(
                    ((&mut pml2[i][0]) as *mut u64) as *mut c_void,
                ) / PAGE_SIZE as u64;
        ...
        pub fn new() -> Self {
            ...
            let phys_addr =
                virtual_address_to_physical_address(ept_state.ept_page_table.unwrap() as _);
            ept_state.ept_pointer = set_bits_value(
                ept_state.ept_pointer,
