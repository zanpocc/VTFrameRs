...
lazy_static! {
    pub static ref HOOK_LIST: ReadWriteLock<Option<HookFunction>> =
        ReadWriteLock::new(Some(HookFunction::new()));
}
...
impl InlineHook {
    pub fn new(rip: *mut u8, new_func: *mut u8) -> Result<InlineHook, &'static str> {
        ...
        unsafe {
            memcpy(
                patch_header as _,
                patch_jmp_thunk.as_mut_ptr() as _,
                jmp_thunk_size as _,
            );
            ...
            memcpy(
                jmp_back as _,
                jmp_back_thunk.as_mut_ptr() as _,
                jmp_thunk_size as _,
            );
        }
    ...
    pub fn hook(&mut self) {
        unsafe {
            let irql = wpoff();
            memcpy(
                self.ori_func_point as _,
                self.patch_header as _,
                self.patch_size as _,
            );
            wpon(irql);
        };
